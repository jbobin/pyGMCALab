...
def HypGMCA(X,n=2,maxts = 7,mints=3,nmax=100,L0=0,UseP=1,verb=0,Init=0,Index_1D=None,Index_2D=None,Aposit=False,BlockSize= None,NoiseStd=[],IndNoise=[],Kmax=0.5,AInit=None,tol=1e-6):
    ...
    S,A = Core_HypGMCA(X=Xw,A=A,S=S,n=n,maxts=maxts,BlockSize = BlockSize,Aposit=Aposit,tol=tol,kend = mints,nmax=nmax,L0=L0,UseP=UseP,verb=verb,IndNoise=IndNoise,Kmax=Kmax,NoiseStd=NoiseStd);
...
def Core_HypGMCA(X=0,n=0,A=0,S=0,maxts=7,kend=3,nmax=100,Index_1D=None,Index_2D=None,BlockSize = 2,L0=1,Aposit=False,UseP=0,verb=0,IndNoise=[],Kmax=0.5,NoiseStd=[],tol=1e-6):
...
#--- Main loop
    while Go_On:
        ...
        for r in range(len(indS)):
            ...
            if UseP == 1:
                Kval = np.min([np.floor(np.max([2./n_X[0],perc*it])*len(indNZ)),n_X[0]-1.])
